...
def send_test_email(request):
    try:
        send_mail(
            ...
            from_email='abhishekmcse03@gmail.com',
            recipient_list=['recipient@example.com'],  # Replace with a real recipient
            fail_silently=False,
        )
    ...
    if usr_loggedin:
        if request.method == 'POST':
            ...
            try:
                ...
                path_file = f'C:/QR/{first_name}.png'
...
class Change_passwd:
    ...
    def send_passwd(self):
        try:
            ...
            # send_mail(self.mail, new_password)  # Uncomment this if you have email functionality
            
            print(f"Generated password: {new_password}")  # Debugging output
            return new_password
        except Exception as e:
...
def ch_pass_unauthenticated(request):
    ...
    if not usr_loggedin and not authenticated:
        if request.method == 'POST':
            ...
            if usr.exists():
                print("User exists")
                change_password = qr_otp.Change_passwd(mail=email)
                new_passwd_value = change_password.send_passwd()

                if new_passwd_value:  # Check if a new password is generated
                    new_passwd = Hash(new_passwd_value)
                    ...
                    messages.warning(request, 'Failed to generate a new password.')
            else:
